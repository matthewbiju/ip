...
4. Run the bundled generator from the repository root:
   ...
   Replace `HEAD`, `WORKTREE`, and the output path with the requested values. The comparison points can be any Git commit-ish such as `HEAD~1`, a tag (e.g. `Level-2`), a branch, or a commit SHA. Use `WORKTREE` for the current files.
...
`scripts/generate-split-view-diff.py` is the bundled standard-library-only generator (no pip installs needed). Keep the page self-contained except for optional syntax-highlighting resources loaded by the page.
